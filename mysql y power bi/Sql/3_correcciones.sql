...
UPDATE `cliente` SET Latitud = (REPLACE(Y,',','.') AS DOUBLE);
UPDATE `cliente` SET Longitud = (REPLACE(X,',','.') AS DOUBLE);
...
UPDATE `empleado` SET Salario = (REPLACE(Salario2,',','.') AS FLOAT);
ALTER TABLE `empleado` DROP `Salario2`;

ALTER TABLE `producto` ADD `Precio` FLOAT NOT NULL DEFAULT '0' AFTER `Precio2`;
UPDATE `producto` SET Precio = (REPLACE(Precio2,',','.') AS FLOAT);
...
UPDATE `sucursal` SET Latitud = (REPLACE(Latitud2,',','.') AS DOUBLE);
UPDATE `sucursal` SET Longitud = (REPLACE(Longitud2,',','.') AS DOUBLE);
...
SELECT 	CURRENT_TIMESTAMP,
		...
		COUNT(*)
FROM (SELECT IdCliente, COUNT(*) FROM `cliente` GROUP BY IdCliente HAVING COUNT(*) > 1) a HAVING COUNT(*) > 0;

INSERT INTO auditoria_carga (FechaHoraCarga, Validacion, CantidadRegistros)
SELECT 	CURRENT_TIMESTAMP,
		'SELECT IdSucursal, COUNT(*) FROM sucursal GROUP BY IdSucursal HAVING COUNT(*) > 1;',
		COUNT(*)
FROM (SELECT IdSucursal, COUNT(*) FROM `sucursal` GROUP BY IdSucursal HAVING COUNT(*) > 1) a HAVING COUNT(*) > 0;
...
SELECT 	CURRENT_TIMESTAMP,
		...
		COUNT(*)
FROM (SELECT IdProveedor, COUNT(*) FROM `proveedor` GROUP BY IdProveedor HAVING COUNT(*) > 1) a HAVING COUNT(*) > 0;

INSERT INTO auditoria_carga (FechaHoraCarga, Validacion, CantidadRegistros)
SELECT 	CURRENT_TIMESTAMP,
		'SELECT IdProducto, COUNT(*) FROM proveedor GROUP BY IdProducto HAVING COUNT(*) > 1;',
		COUNT(*)
FROM (SELECT IdProducto, COUNT(*) FROM `producto` GROUP BY IdProducto HAVING COUNT(*) > 1) a HAVING COUNT(*) > 0;
